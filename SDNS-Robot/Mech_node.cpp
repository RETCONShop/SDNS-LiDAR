...
#include "snl_messages/msg/mech.hpp"
...
public:
	MechNode() : Node('snl_mechnode')
	{
		...
		publisher = this->create_publisher<snl_messages::msg::Mech_imu>("mech_imu", 10);
...
private:
	void imu_callback(const snl_messages::msg::Imu::SharedPtr imu)
	{
		...
		uncal_gyro(1) = imu->gyro.x;
		uncal_gyro(2) = imu->gyro.y;
		uncal_gyro(3) = imu->gyro.z;

		rclcpp::Time currentTime;
		std::chrono::milliseconds currentTimeMs;
		auto imuMechMessage = snl_messages::msg::Mech_imu();
		Vector3d w_t__t_b;
		Matrix3d temp;
		...
		double wttb_mag = w_t__t_b.dot(w_t__t_b)[0];
		...
		imuMechMessage.timestamp = currenntTimeMs;
		...
		//Publish Message
		publisher->publish(imuMechMessage);
	}
	...
	{
		g_0 = 9.7803253359 * (1.0f + 0.001931853f * pow(sin(L_b), 2)) / sqrt(1 - pow(e, 2) * pow(sin(L_b), 2));
		g_n__bD = g_0 * (1.0f - (2.0f / R0) * (1.0f + flat * (1 - 2 * pow(sin(Lat_Prescott), 2)) + (pow(w_ie, 2) * pow(R0 , 2) * Rp) / mu) * h_b + 3.0f * pow(h_b, 2) / (pow(R0, 2)));
		return g_n__bD;
	}
	...
	rclcpp::Publisher<snl_messages::msg::Mech_imu>::SharedPtr publisher;
